var CandidatosPanel_view = Backbone.View.extend({
    ...
    ,events:{
        ...
        "click .btn_finalizar_cuestionario": "f_guardarRespuestasCuestionario",
        "click .btn_guardar_datos_personales":"f_guardar_datos_personales",
        "click .btn_guardar_cambio_pass":"f_cambiar_password"
    },
    ...
    f_guardarRespuestasCuestionario: function(e){
        var _this = this;
        lo_currentItem = e.currentTarget;
        id_vacate_cuestionario = $(lo_currentItem).data('hash');
        li_tipo_cuestionario = $(lo_currentItem).data("tqy");

        var la_tiposCuestionario = [".panel-nach-pregunta", ".panel-nach-pregunta-ortografia", ".panel-nach-pregunta-typing", ".panel-nach-pregunta"];
        var la_respuestas = _this.f_validarCuestionario(la_tiposCuestionario[li_tipo_cuestionario]);
        ...
        la_dataCollection = {
            respuestas: la_respuestas
        }

        console.log(la_dataCollection);
        
        
        _this.model.f_guardarRespuestasCuestionarioModel(la_dataCollection,id_vacate_cuestionario).done(function (response){
            ...
            if(response.return == 1){
                location.href = BASE_URL+'mis-postulaciones/ver-vacantes?r=success';
            }else{
    ...
    f_pausaPruebaTyping: function(e){
        ...
        localStorage.setItem("finaliza_prueba_"+li_id_pregunta, ls_horaFinaliza);
    },
    ...
    f_comienzaPruebaTyping: function(e){
        ...
        localStorage.setItem("comienza_prueba_"+li_id_pregunta, ls_horaEmpieza);
    },
        ...
        if(li_total_preguntas > 0){
            for(li_index = 0; li_index < li_total_preguntas; li_index++){
                ...
                la_respuesta = {
                    ...
                    hora_inicia: localStorage.getItem("comienza_prueba_"+li_id_pregunta),
                    hora_fin: localStorage.getItem("finaliza_prueba_"+li_id_pregunta)
                }
    ...
    f_guardar_datos_personales:function(e){
        var _this = this;
        ...
        la_data = {
            ...
            sexo:       $("#genero_sexual").val(),
            rfc:        $("#rfc").val(),
            numero:     $("#numero_telefono").val(),
            correo:     $("#correo_electronico").val()
        }

        _this.model.f_cambios_personales_model(la_data).done(function (response){
            if(response.return > 0){
                swal("Bien!","Tus datos fueron actualizados correctamente","success");
                setInterval(actualizar,3000);
            }else{
                ("Error","Tus datos fueron no actualizados correctamente,favor de verificar","error");
            }
        });

        function actualizar(){
            location.href = BASE_URL+'mi-cuenta/datos-personales';
        }
    },

    f_cambiar_password:function(){
        var _this = this;
        la_data = {
            anterior:$("#pass_anterior").val(),
            nueva:$("#pass_nueva").val(),
            repetir:$("#pass_repetir").val()
        }

        elementos = ['pass_anterior','pass_nueva','pass_repetir'];

        if(la_data.anterior == "" || la_data.nueva == "" || la_data.repetir == "" ){
            swal("Aviso","Deber llenar todos los campos pintados en color rojo","warning");
        ...
        _this.model.f_cambiar_password_model(la_data).done(function (response){
            console.log(response);

            if(response.return > 0){
                swal("Bien!","Tu contraseña a sido cambiada correctamente","success");
                setInterval(actualizar,3000);
            }else{
        ...
        function actualizar(){
            location.href = BASE_URL+'mi-cuenta/cambiar-password';
        }
